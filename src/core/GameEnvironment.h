...
// Best guess at the WoW Logs directory when the user has not chosen one.
std::filesystem::path ResolveDefaultWowLogDirectory();

// "<install>/_retail_/Logs" -> "<install>". Empty when the path is not shaped
// like a WoW log directory.
...
// auto-detected install when wowLogDirectory does not resolve.
